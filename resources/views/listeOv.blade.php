...
<style>
    ...
    .site-badge {
        ...
        color: #1e3c72; background: #e8eef8;
        border: 1px solid #c5d3ed; border-radius: 20px;
...
<div class="container py-5">
    ...
    @foreach (['success' => 'alert-success', 'error' => 'alert-danger', 'warning' => 'alert-warning'] as $key => $class)
        @if(session($key))
            <div class="alert {{ $class }} alert-dismissible fade show" role="alert" id="alert">
                <i class="bi bi-{{ $key === 'success' ? 'check-circle-fill' : 'exclamation-triangle-fill' }} me-2"></i>
                {!! session($key) !!}
                    ...
                    <tbody>
                        @forelse($souscripteurs as $s)
                            ...
                            @php
                                ...
                                $ovCreditReel     = null;
                                $ovCreditDiff     = null;
                                $ovsDoneNormaux   = collect();
                                $diffCredit       = 0;
                                $paiementParCredit = false;
                                $totalOvsCredit   = 0;
                                $nbOvsCrees       = 0;
                                $tousOvsGeneres   = false;
                                ...
                                if ($creditEnregistre) {
                                    ...
                                    $ovCreditDiffPaye = $ovCreditDiff?->paiement !== null;
                                ...
                                $ovT2NormaleExiste = !$creditEnregistre
                                    && $s->ovs->whereNull('type_ov')->where('numero_tranche', 2)->isNotEmpty();

                                /* Peut-on générer un nouvel OV ? */
                                $peutGenerer = true;
                                if ($programme === 'LPA') {
                                ...
                                if ($programme === 'LSP') {
                                    ...
                                    $reste        = max(0, $prixLogement - $s->aides->sum('montant') - $s->ovs->sum('montant_paye'));
                                    $peutGenerer  = $reste > 0;
                                }
                                if ($programme === 'LPL') {
                            ...
                            <tr>
                                    ...
                                    @if($nomSite)
                                        <div>
                                            <span class="site-badge">
                                                <i class="bi bi-buildings"></i> {{ $nomSite }}
                                            </span>
                                        </div>
                                    @endif
                                    <div class="localisation-bloc">
                                        <div><i class="bi bi-building text-muted me-1"></i> Bât. <strong>{{ $s->logement->num_batiment }}</strong></div>
                                        <div><i class="bi bi-layers text-muted me-1"></i> Étage {{ $s->logement->num_etage }}</div>
                                        <div><i class="bi bi-door-open text-muted me-1"></i> Porte {{ $s->logement->num_porte }}</div>
                                    </div>
                                </td>
                                ...
                                <td class="p-3">
                                    @forelse($s->ovs as $ov)
                                        @php
                                            $ovPos = $s->ovs->values()->search(fn($o) => $o->id === $ov->id) + 1;
                                        @endphp
                                        <div class="ov-item mb-3 p-3 border rounded-3 shadow-sm">

                                            {{-- En-tête : badges type + statut paiement --}}
                                            ...
                                            <div class="d-flex gap-2 flex-wrap justify-content-center">
                                                ...
                                                {{-- Modifier (chef_service_com uniquement) --}}
                                            ...
                                            @if($creditEnregistre)
                                                <div class="d-flex align-items-center gap-2 flex-wrap">
                                                    ...
                                                    @if($paiementParCredit)
                                                        <span class="badge bg-success">
                                                            <i class="bi bi-check-all me-1"></i> Dossier Soldé
                                                        </span>
            ...
            @if($souscripteurs->hasPages())
                ...
                <div class="pagination-wrapper d-flex justify-content-between align-items-center px-4 py-3 border-top">
                    <div class="pagination-info">
                        Affichage de <strong>{{ $souscripteurs->firstItem() }}</strong>
                        à <strong>{{ $souscripteurs->lastItem() }}</strong>
                        sur <strong>{{ $souscripteurs->total() }}</strong> souscripteurs
                    </div>
